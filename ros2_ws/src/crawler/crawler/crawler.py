...
class Crawler(Node):
    ...
    ## The constructor.
    def __init__(self):
        super().__init__('crawler_node')
        self.MR = Motor(config.motor_right_IO2, config.motor_right_DIR, config.motor_right_PWM)
        self.ML = Motor(config.motor_left_IO2, config.motor_left_DIR, config.motor_left_PWM)
        self.init_IO2_DIR()
        self.init_PWM()
        #self.init_light()
        self.get_logger().info('Crawler node initialized')
    ...
    def PWM(self, on_off):
        if self.MR.enable_PWM(on_off, config.motor_right_PWM) == True:
            if self.ML.enable_PWM(on_off, config.motor_left_PWM) == True:
                ...
                print("PWM activation error for motor_left_IO2")
                return False 
        else: 
            print("PWM activation error for motor_right_IO2")
            return False 
    ...
    def forward(self, duty_cycle):
        if self.MR.DIR(config.motor_right_DIR, 0) == True:
            if self.ML.DIR(config.motor_left_DIR, 1) == True:
                if self.MR.duty_cycle(0, config.motor_right_PWM) == True:
...
def main(args=None):
    ...
    # Example usage:
    crawler.forward(50)  # Move forward at 50% speed
